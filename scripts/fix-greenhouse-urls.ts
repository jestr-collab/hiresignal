...
async function headOk(targetUrl: string): Promise<boolean> {
  try {
    const res = await fetch(targetUrl, {
      ...
      redirect: "follow",
      headers: {
        "user-agent": "HireSignal-url-fix/1.0",
      },
    });
...
async function main() {
  const { data: companies, error } = await supabase
    .from("companies")
    .select("id, name, domain, greenhouse_url")
    .order("name");
  ...
  for (const company of list) {
    const { id, name, domain, greenhouse_url: existing } = company;

    if (existing?.trim()) {
      const ok = await headOk(existing.trim());
      if (ok) {
        console.log(`[found] ${name}: existing URL OK (${existing.trim()})`);
        continue;
      }
    }
